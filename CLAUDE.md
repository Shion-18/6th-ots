...
## スタック
Next.js 16（App Router）/ React 19 / TypeScript 5 / Tailwind CSS 4 / Supabase（Postgres）
...
- `lib/` — session.ts（HMAC認証）, api-validation.ts（Zod）, rate-limit.ts, team-storage.ts（Supabase API + localStorage fallback）
